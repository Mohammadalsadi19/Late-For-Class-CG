const Game = (() => {
  ...
  function init(_canvas, _professorData, _difficulty, _onWin, _onLose) {
    ...
    world       = Maze.generate(COLS, ROWS, TILE_SIZE);
    activeEvent = Events.pickRandomEvent();
    Events.applyEvent(activeEvent, world);
    ...
    world.obstacleSeeds.forEach(seed => {
      ...
      if (seed.type === 'student') {
        ...
        const patrol = world.patrolPaths.find(
          p => p.seedCol === seed.col && p.seedRow === seed.row
        );
  ...
  function _findProfessorSpawn(world) {
    ...
    for (let r = 0; r < world.rows; r++) {
      for (let c = 0; c < world.cols; c++) {
        ...
        if (tile === world.TILE.WALL || tile === world.TILE.DOOR) continue;
        if (c === world.spawnCol && r === world.spawnRow) continue;
        if (c === world.goalCol  && r === world.goalRow)  continue;
    ...
    for (const cell of walkable) {
      if (Utils.manhattan(cell.col, cell.row, world.spawnCol, world.spawnRow) > 10)
        return cell;
    }
    return walkable[0];
  }
  ...
  function _getDifficultySettings(level) {
    const settings = {
      easy:   { level:'easy',   label:'Easy',   timeScale:1.25, playerSpeedScale:1.04, obstacleSpeedScale:0.85 },
      normal: { level:'normal', label:'Normal', timeScale:1,    playerSpeedScale:1,    obstacleSpeedScale:1    },
      hard:   { level:'hard',   label:'Hard',   timeScale:0.85, playerSpeedScale:0.98, obstacleSpeedScale:1.12 },
    };
  ...
  function update(dt, input) {
    ...
    doors.forEach(door => {
      ...
      door.update(dt, dist < TILE_SIZE * 1.8, input.space);
    });
  ...
  if (Algorithms.aabbOverlap(player.aabb, pu.aabb)) {
    ...
    if (pu.type === 'coffee') {
      ...
      _requestBFS();
    } else if (pu.type === 'call') {
      professorAI.delay(4);
    } else if (pu.type === 'roadblock') {
      professorAI.slowDown(6);
    } else if (pu.type === 'traffic') {
      professorAI.freeze(3);
    }
  }
});
  ...
  function _movePlayerAxisX(dt) {
    ...
    _resolvePlayerWalls('x');
  }

  function _movePlayerAxisY(dt) {
    player.y += player.vy * dt;
    _resolvePlayerWalls('y');
  }

  function _resolvePlayerWalls(axis) {
    const ts  = TILE_SIZE;
    const pr  = player.r;
    ...
    for (let dr = -2; dr <= 2; dr++) {
      for (let dc = -2; dc <= 2; dc++) {
        ...
        if (world.grid[tr][tc] !== world.TILE.WALL) continue;

        const result = Algorithms.circleRect(player.circle, { x: tc*ts, y: tr*ts, w: ts, h: ts });
        if (result.hit) {
          player.x += result.nx * result.depth;
          player.y += result.ny * result.depth;
          if (axis === 'x' && player.vx * result.nx > 0) player.vx = 0;
          if (axis === 'y' && player.vy * result.ny > 0) player.vy = 0;
        }
      }
    }
  ...
  function _resolveProfessorWalls(prof) {
    ...
    for (let dr = -1; dr <= 1; dr++) {
      for (let dc = -1; dc <= 1; dc++) {
        ...
        const result = Algorithms.circleRect(prof.circle, { x: tc*ts, y: tr*ts, w: ts, h: ts });
        if (result.hit) {
    ...
    prof.x = Utils.clamp(prof.x, prof.r, world.cols * ts - prof.r);
    prof.y = Utils.clamp(prof.y, prof.r, world.rows * ts - prof.r);
  }
  ...
  function _requestBFS() {
    const playerCell = Utils.pixelToGrid(player.x, player.y, TILE_SIZE);
    const bfsGrid    = world.grid.map(row => row.map(t => (t === world.TILE.WALL ? 1 : 0)));
    const result     = Algorithms.bfs(bfsGrid, playerCell.col, playerCell.row, world.goalCol, world.goalRow);
    bfsResult        = result;
    bfsAnimFrame     = 0;
  }
  function draw() {
    if (!canvas || !ctx) return;
    Renderer.drawFrame(ctx, canvas, {
  ...
  function toggleDebug()   { debugMode = !debugMode; return debugMode; }
  function setRunning(v)   { running = v; }
  function isRunning()     { return running; }
  function handleResize()  { if (canvas) _resizeCanvas(); }
  ...
  return {
    ...
    getActiveEvent,
    toggleDebug, setRunning, isRunning, handleResize
  };
})();